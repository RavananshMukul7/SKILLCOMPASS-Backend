import { createGitHubAuthorizationUrl, handleGitHubCallback, } from "./github.service.js";
...
export const connectGitHub = async (req, res) => {
    ...
    const authorizationUrl = await createGitHubAuthorizationUrl(req.user.id);
...
export const githubCallback = async (req, res) => {
    ...
    console.log("GitHub callback URL:", req.originalUrl);
    ...
    if (error) {
        throw new AppError(`GitHub authorization failed: ${errorDescription ??
            error}`, 400);
    }
    if (!code || !state) {
        throw new AppError("Missing GitHub authorization parameters", 400);
    }
     ...
     */
    res.redirect("http://localhost:3000/settings/github");
};
