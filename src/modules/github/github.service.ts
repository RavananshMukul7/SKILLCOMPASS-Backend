...
import {
  findSkillCompassInstallation,
} from "./githubAppAuth.service.js";

const OAUTH_STATE_DURATION_MS =
...
export const handleGitHubCallback =
  async (
    ...
    const oauthState =
      await prisma.gitHubOAuthState.findUnique({
        where: {
          stateHash,
        },
      });
     ...
     */
    const consumedState =
      await prisma.gitHubOAuthState.updateMany({
        where: {
          id: oauthState.id,
          consumedAt: null,
          expiresAt: {
