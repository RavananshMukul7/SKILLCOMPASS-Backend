...
export const detectTechnologiesWithAI = async (input) => {
...
${file.content}
`.trim();
        const response = await ollamaModel.generateJson({
            systemPrompt,
            userPrompt,
            temperature: 0,
            maxTokens: 150,
        ...
        for (const technologyName of parsed.technologies) {
            ...
            const alreadyDetected = detectedTechnologies.some((item) => item.normalizedName === normalizedName);
            if (alreadyDetected) {
                continue;
            }
