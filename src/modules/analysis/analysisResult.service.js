...
export const getAnalysisResult = async (userId, analysisRunId) => {
        ...
        snapshot: snapshot
            ? {
                ...
                languages: snapshot.languages.map((language) => ({
                    ...
                    percentage: Number(language.percentage),
                })),
                technologies: snapshot.technologyEvidence.map((technology) => ({
                    id: technology.id,
                    ...
                    confidence: Number(technology.confidence),
                })),
            }
            : null,
        skills: analysisRun.userSkills.map((userSkill) => {
            const assessment = userSkill.assessments[0];
            return {
                id: userSkill.id,
                ...
                evidence: userSkill.evidence
                    .filter((evidence) => snapshot?.technologyEvidence.some((technology) => technology.id ===
                    evidence.technologyEvidenceId))
                    .map((evidence) => ({
                    technology: evidence.technologyEvidence
                        .technologyName,
                    evidenceType: evidence.technologyEvidence
                        .evidenceType,
                    evidenceValue: evidence.technologyEvidence
                        .evidenceValue,
