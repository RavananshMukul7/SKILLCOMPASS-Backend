...
const detectRequirementType = (text, skillPatterns) => {
    ...
    for (const rawLine of lines) {
        ...
        const isPreferredSection = PREFERRED_SECTION_PATTERNS.some((pattern) => pattern.test(line));
        const isRequiredSection = REQUIRED_SECTION_PATTERNS.some((pattern) => pattern.test(line));
        ...
        const skillAppearsOnLine = skillPatterns.some((pattern) => pattern.test(line));
        if (!skillAppearsOnLine) {
            continue;
        }
        const hasPreferredInline = PREFERRED_INLINE_PATTERNS.some((pattern) => pattern.test(line));
        const hasRequiredInline = REQUIRED_INLINE_PATTERNS.some((pattern) => pattern.test(line));
...
export const extractJobSkills = (title, description) => {
    const text = `${title}\n${description ?? ""}`;
    ...
    for (const rule of SKILL_RULES) {
        ...
        const requirementType = detectRequirementType(text, rule.patterns);
        extracted.set(rule.normalizedName, {
            skillName: rule.skillName,
            normalizedName: rule.normalizedName,
            category: rule.category,
            requirementType,
            importance: requirementType === "PREFERRED"
