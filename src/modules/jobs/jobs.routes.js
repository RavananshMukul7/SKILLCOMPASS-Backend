...
/* =========================================================
   ...
   ========================================================= */
jobsRouter.post("/sources/:sourceName/sync", requireAuth, async (req, res, next) => {
    try {
...
jobsRouter.get("/recommended", requireAuth, async (req, res, next) => {
    try {
        ...
        const minScore = typeof minScoreParam === "string" ? Number(minScoreParam) : 0;
...
/* =========================================================
   ...
   ========================================================= */
jobsRouter.post("/recommended/generate", requireAuth, async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            res.status(401).json({
...
jobsRouter.get("/:jobId", requireAuth, async (req, res, next) => {
    try {
        ...
        const gaps = typeof analysisRunId === "string"
