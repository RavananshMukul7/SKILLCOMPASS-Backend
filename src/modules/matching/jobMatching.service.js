...
const getRequirementMultiplier = (requirementType) => {
    return requirementType === "PREFERRED"
        ? PREFERRED_WEIGHT_MULTIPLIER
        : REQUIRED_WEIGHT_MULTIPLIER;
};
const buildExplanation = (totalSkills, skillsMeetingThreshold, gaps) => {
    const base = `The candidate meets the configured proficiency threshold for ` +
    ...
    const gapDetails = gaps
        ...
        .join(", ");
    return (`${base} ` +
        `Missing or below-threshold skills: ${gapDetails}.`);
};
export const calculateJobMatch = async (userId, jobId, analysisRunId) => {
    ...
    if (!analysisRun) {
        throw new Error("Completed analysis run not found for the authenticated user.");
    }
    ...
    const userScoreBySkillId = new Map();
    for (const userSkill of userSkills) {
        userScoreBySkillId.set(userSkill.skillId, Math.max(0, Math.min(100, round(toNumber(userSkill.currentScore), 3))));
    }
    ...
    for (const jobSkill of job.skills) {
        ...
        const requirementMultiplier = getRequirementMultiplier(jobSkill.requirementType);
    ...
    if (totalRequirementWeight <= 0) {
        throw new Error("Job skill weights must contain at least one positive importance value.");
    }
    const matchScore = round((weightedSkillScore / totalRequirementWeight) * 100, 3);
    const skillCoverage = round(achievedRequirementWeight / totalRequirementWeight, 4);
    const explanation = buildExplanation(job.skills.length, job.skills.length - gaps.length, gaps);
